...
function cases($data)	{
	...
	foreach($data as $record){		
		...
		elseif (in_array($assign_status,array("aprove_raise_query","Aprove_raise_query","APROVE_RAISE_QUERY"))) {
			...
			$response=array();
			$db = connect_db();
			$sql = "SELECT case_query_id,qc_case_query,qc_query_file,case_assignment_id,qc_query_on FROM case_queries where case_assignment_id='$caseassignid'";
			$exe = $db->query($sql);
			$data = $exe->fetch_all(MYSQLI_ASSOC);
			$db = null;
			$response=$data;
...
function checkfeestatus($conid,$status,$caseassignid){
	...
	//and case_assignment_id='$caseassignid'
	$exe = $db->query($sql);
	$data = $exe->fetch_all(MYSQLI_ASSOC);
	$db = null;
	foreach($data as $record){		
...
function fees($data){
	...
	$feestats=checkfeestatus($conid,$status,$caseassignid);
	$response=array();
	if ($feestats==2) {
		$db = connect_db();
